...
#什么是动态规划的思路？
如何利用上一段的解来辅助求解当前的最优解，也就是上一段的最优解和可以推导出当前最优解。这个和数学归纳法的思路在某种程度上有些相似之处。
...
1. 最简单的思路就是每一天试图和前一天比较然后交易。如果两天之间的趋势是上涨的，那么就进行一次买卖
2. 在这个基础上优化的方式就是不每次看到上涨都交易一次，而是保留一个状态叫做是否持有股票，直到看到一个下降趋势的时候才进行一次交易。
...
1. 这里需要比第一题运用多一层动态规划的思路，即第一次交易和第二次交易的分割点变动。也就是将数组分成两段，切割的断点从1-n循环，保存在向量中，每一段都利用上一段的最优解找本段的最优解。
2.这个题目可以进一步扩展到最多k次交易。
f(i, k) = MAX(f(i-1, k), price[i] - price[j] + f(j, k-1)) 其中0<=j<i。分别对i, k, j做循环。

**错误**

1. 注意交易点的选择还是很容易在细节上出错。不一定非要做到完全对称也同样可以。
2. 从左到右寻找的时候是保留左边最小的值，看本天是不是一个好的卖出机会；反过来以右边开始的时候，是保留右边最大的值，看本天是不是一个好的买入的机会。


###134. [Gas Station](https://leetcode.com/problems/gas-station/)
```贪婪算法	```

gas和cost分别代表i这一站可以加到的油的数量以及到达i+1站所耗费的油的数量，问从哪一站出发可以循环一周。如果不能返回-1。


###135. [Candy](https://leetcode.com/problems/candy/)
```贪婪算法	```

给定一组孩子和他们的rating。规定rating高的孩子要比他的邻居的糖果数量要多，而且每个孩子至少必须拿到一颗糖。问最少需要多少颗糖。

**错误**

1. 不可以使用减少的方法。一开始构想的是，遇到rating高的多给，遇到rating小的拿走一个，最后再让大家都有一颗糖填平。但是这样做的结果是不对的。比如[1, 2, 2]或者[2, 2, 1]这样的序列。问题都出在怎么处理等值的情况。在减少的时候，如果对等值减少有可能会导致最小值变化，最后要补平的时候反而导致了整体需要发的糖果数量的增加。所以最后使用的做法是，从左向右发一次，先保证从左到右是不降序数列，也就是糖果数量一定满足左侧孩子的排分，然后再从右侧重新看一次，微调到右侧同样满足
2. 在比较评分的时候注意脚标。
...
给定一个数组，其中除了一个数字，剩下的数字均出现了两次，寻找只出现了一次的数。
...
1. 与上一题不同无法直接使用异或这样具有特殊特征的计算。但是可以用同样的思路，即从位的角度看，出现3次的数，一定在这一位上出现过3次1。由此，可以知道如果我们按照位把所有的数加和起来之后模3，所有出现3次的数的结果就都被消去，只剩下了出现了一次的数字的结果。
